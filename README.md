...
실제 실행은 상태를 읽기 전에 해당 파일 경로의 OS 잠금을 얻고, 조회·발송·저장·Git 게시가 끝날 때까지 유지합니다. 같은 로컬 상태 파일을 사용하는 다른 실행이 있으면 기다리지 않고 종료 코드 1로 중단합니다. 상태 경로는 실제 경로로 정규화하며, Windows는 대소문자 차이도 통합합니다. 조회만 하는 `--dry-run`은 잠금 파일을 만들지 않습니다.

잠금 파일 `.<상태파일명>.execution.lock`은 상태 파일 옆에 남는 빈 파일이며 Git에서 제외됩니다. **실행 중 잠금 파일을 삭제하거나 교체하지 마세요.** 파일이 남아 있어도 잠겨 있다는 뜻은 아닙니다. 정상 종료와 프로세스 종료 시 OS가 잠금을 해제하며, PID나 경과시간을 추정해 잠금을 강제로 빼앗지 않습니다. 권한 오류나 지원되지 않는 OS에서는 실행을 중단합니다. 이 잠금은 같은 로컬 경로와 잠금 규칙을 따르는 실행끼리 적용되며, 다른 PC·별도 clone·잠금을 사용하지 않는 외부 작성자를 조정하지 않습니다.
